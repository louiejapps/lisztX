...
${dotsMenu}
			...
			</td>`; rrow.innerHTML = rowData; let thumbsBut; rrow.querySelector("#thumbs") && rrow.querySelector("#thumbs").addEventListener("click", function (e) { setViewer(childData.key, sessionNumber), commentModal(childData) }), tablebody.appendChild(rrow), rrow.querySelector("#menu-options").addEventListener("click", function (e) { var t; menuModal(e.clientX, e.clientY, childData) }); let reactDiv = rrow.querySelector("#react-div"); reactDiv.addEventListener("click", function (e) { var t; reactModal(e.clientX, e.clientY, childData, reactDiv) }), rrow.querySelector("#comment-div").addEventListener("click", function () { setViewer(childData.key, sessionNumber), commentModal(childData) }), document.title = "Liszt | " + user, loading.style.display = "none"
		})
	}, function (e) { console.error("Failed to retrieve quotes:", e), "PERMISSION_DENIED" === e.code ? notif.innerHTML = "Database is locked" : "NETWORK_ERROR" === e.code ? notif.innerHTML = "No internet connection." : notif.innerHTML = "Failed to save link." })
} function setViewer(e, t) { database.ref(`quotes/${e}/views/${t}`).set(!0) } function saveData(e, t, A, a, i) { return inputChanged = !1, saveButton.disabled = !0, !((a.match(/\n/g) || []).length > 5) && ((t.length > 50 && (t = t.substring(0, 50)), navigator.onLine) ? database.ref("quotes").push().set({ quote: limitCharacters(e, 200), uname: t, thumbnail: A, sessionkey: internetTime, pinned: !1, show: "true", views: "0", visits: "0", background: i, caption: a, subtitle: "Guest", timestamp: firebase.database.ServerValue.TIMESTAMP }, function (e) { e ? (console.error("Failed to save quote:", e), notif.style.display = "block", "PERMISSION_DENIED" === e.code ? notif.innerHTML = "Database is locked" : "NETWORK_ERROR" === e.code ? notif.innerHTML = "No internet connection." : notif.innerHTML = "Failed to save link.") : (myForm.style.display = "none", postFrom.reset(), showFormButton.style.display = "block", myContent.style.display = "block", notif.style.display = "block", notif.innerHTML = "Shared Successfully!", setTimeout(function () { notif.innerHTML = "" }, 5e3), saveButton.disabled = !1) }) : (notif.style.display = "block", notif.innerHTML = "No Connection"), !0) } function toggleLike(e, t, A) { for (let a = 0; a < reactArr.length; a++)document.getElementById(`${reactArr[a]}`).style.fontSize = "1em"; var i = database.ref(`quotes/${t}/react/${A}`); if ("" === e) i.remove(); else { i.set(e); let o = document.getElementById(`${e}`); o && (o.style.fontSize = "2em") } } function getReactCount(e, t) { var A = 0; return firebase.database().ref(`quotes/${e}/react`).orderByValue().equalTo(t).once("value", function (e) { A = e.exists() ? e.numChildren() : 0 }), A } function getCount(e, t) { var A = 0; return firebase.database().ref(`quotes/${e}/${t}`).once("value", function (e) { A = e.exists() ? e.numChildren() : 0 }), A } function generateReactionHTML(e) {
	let t = Object.entries(e).sort(([, e], [, t]) => t - e).reduce((e, [t, A]) => ({ ...e, [t]: A }), {}), A = `
    <div
	...
	`; function a(e) { let t = { loves: "❤️", likes: "\uD83D\uDC4D", wows: "\uD83D\uDD25", hahas: "\uD83D\uDE02", frowns: "\uD83D\uDE25", dislikes: "\uD83D\uDC4E" }; return t[e] } return A
} function getTotalReact(e) { var t = 0; return firebase.database().ref(`quotes/${e}/react`).once("value", function (e) { e.exists() && (t = e.numChildren()) }), t } function updatelike(e, t) { let A = document.getElementById(`${e}`); A && (0 === t ? A.innerText = " " : A.innerText = `${t}`) } function togglePin(e) { database.ref(`quotes/${e}/pinned`).transaction(e => !e) } function setComment(e, t, A, a) { console.log(a); let i = database.ref(`quotes/${e}/comments`), o = `<img src="mage/${a}.png" height="75px">`; "" !== a && (A = o), i.push({ username: t, message: A, timestamp: firebase.database.ServerValue.TIMESTAMP }) } function loadComments(e, t) {
	t.innerHTML = ""; let A = ""; A = window.matchMedia("(orientation: landscape)").matches ? "max-width: 400px" : "max-width: 300px"; let a = database.ref(`quotes/${e}/comments`); var i = []; a.orderByChild("timestamp").on("value", function (e) { e.forEach(function (e) { var t = e.val(); t.key = e.key, i.push(t) }), i.reverse() }), i.forEach(function (i) {
		var o = document.createElement("tr"); o.innerHTML = `<td>
		...
		</td>`, t.appendChild(o), o.querySelector("#del-comment").addEventListener("dblclick", function () { event.preventDefault(), a.child(i.key).remove().then(() => { loadComments(e, t), console.log("Data successfully deleted") }).catch(e => { console.log("Error deleting data:", e) }) })
	})
} showFormButton.addEventListener("click", function () { postModal() }), searchBt.addEventListener("click", function () { var e = document.getElementById("search-in").value; moreButton.innerHTML = "<b>END OF SEARCH RESULTS</b>", more_toggle = !0, showFormButton.innerHTML = '<b>Search results for "' + e + '"</b>', window.location.href = "#top", column1.style.backgroundColor = "#f2cbc1", loadDatabase(1e3, e, !1), document.getElementById("search-in").value = "" }), window.addEventListener("beforeunload", function (e) { inputChanged && (e.preventDefault(), e.returnValue = "") }), moreButton.addEventListener("click", function () { more_toggle ? (window.location.href = "#top", location.reload()) : loadDatabase(items += 9, "", !1, quoteTableBody) }), document.getElementById("banner").addEventListener("dblclick", function (e) { resetPage() });
